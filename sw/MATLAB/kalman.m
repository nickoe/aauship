...
for n = 2:N;
       ...
       Wn(:,n) = [randn(3,1);randn(1,1);0;randn(3,1);0].*SqM';
...
for n = 2:N;
 ...
 packLostL(:,n) = packLost(:,n);   % Looses 5 percent of the packages. 
    ...
    XL(find(packLost(:,n)==0),n) = XL(find(packLost(:,n)==0),n-1);
              if sLL == GPS_freq && packLostL(2,n) == 1;
                 ...
                 XLL(find(packLostL(:,n)==0),n) = XLL(find(packLostL(:,n)==0),n-1);
              end
