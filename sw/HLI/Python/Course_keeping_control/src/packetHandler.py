...
'''	def checksum(self,packet):
	...
	def parsePacket(self,packet): 
		...
		#print 'success'
		print self.newdata[len(self.newdata)-1] #Replaced by PackerParser.py
		
	#Function which reads the new packet :
	def parser(self,array):
		#Array should always, as a minimum, contain the length of the data package.
		success = False
		packet = []
		length=1
		try:
			length = len(array)			#Attempt to get the length of the array
			for j in range(length):		#Append the read values to the packet.
				packet.append(array[j])
		except:							#Except if the array is an integer (only the length)
			packet.append(array)		#Append the read length to the packet
			#extrabits = 4
