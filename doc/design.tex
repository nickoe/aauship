...
\item[PWM]\hfill \\ outputs for actuators. Both some PWM outputs supposed to be used with servos and \ac{ESC}'s used in RC hobby, together with som full range and kilo hertz range frequncies, which could be used for modulating H-bridge dreviers directly.
\item[I$^2$C]\hfill \\ option for aux comminication \todo{hmm, maybe I actually forgot to wire this to the interface connector}
